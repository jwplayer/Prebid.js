    ...
    visitor: {
      Program(path, state) {
        ...
        if (modName != null) {
          ...
          path.node.body.push(...api.parse(`const installedModules = window.${pbGlobal}.installedModules || []; installedModules.push('${modName}');`,  {filename: state.filename}).program.body);
        }
      },
      StringLiteral(path) {
